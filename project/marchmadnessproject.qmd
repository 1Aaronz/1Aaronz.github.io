...
March Madness is an annual tournament for Division 1 (DI) college basketball. I will be training prediction models to assign a probability to teams winning in a match-up. 
...
Advanced efficiency metrics was sourced from a community dataset compiled by Pilafas. I opted out of using Pomeroy's cite directly, because the site iteratively updates with post-tournament results, which would result in data leakage.

Public pick data was pulled from the mRchmadness package, which scrapes pick data from ESPN.

## Loading the data

There is a large number of csv files that could be useful for the analysis, so I load all the data into a directory to pull each csv as needed
...
This is the results of the cleaning. We now have a match-up (Team A against Team B) and a probability for each team year. 
```{r}
#| echo: true
...
For linear regression, we use the first two principle components resulting from the principal component analysis of all the predictor variables.
...
$p(Y=1 \mid X_1, X_2)$ indicates the probability team 1 wins the match-up, given the difference in predictor values for each team.
...
For this study, Elastic Net is used to estimate match-up win probabilities while reducing multi-collinearity among predictor variables and preventing over-fitting.

### Training the models

For each year the model is to predict, the model gets retrained on all previous years. For example, the model trained to predict 2024 gets trained on all data from 2003-2023, the 2025 model gets trained on 2003-2024, up to 2026. 
...
In order for the public pick data to be meaningful, it needs to be converted from frequency of teams picked to each round of the tournament into probability of team A beating team B.
...
These simulations generate a random 63 game bracket based on the probabilities given by each model, which is then repeated 100 times for each model on each year we predict. 
...
### Variance explained and Principle Component bindings
...
PC1 seems to correlate with team strength. PC2 seems to correlate with pace of play.
...
The findings suggest that out of the 63 games in the NCAA tournament, a gradient boosted tree model is, on average, the best on average at evaluating the probability that a team will win a given game. When looking at just the championship games for each year, the public consensus model on average is the best at picking champions. This increased championship accuracy causes the consensus model to outperform machine learning models in bracket generation in some years.

However, the championship model is overconfident in its predictions, which can skew the results of the bracket simulations. Championship accuracy has a high deviation each year. In 2023, the consensus model failed to pick the championship game correctly a single time out of 100 simulations. In contrast the best model in 2023 picked the championship game correctly 36 times out of 100 simulations. In 2024, the consensus model was nearly twice as accurate as the next most accurate model that year (50% versus 27%). This suggests that when the consensus model is overconfident in a champion and the champion wins, the consensus model outperforms the machine learning models. When the favorite doesn't win, the consensus performs very poorly for that year. This explains the high variance in performance from the consensus year to year.

The current findings suggest that using a gradient boosted model for the first 62 games of the tournament, and the consensus model for the championship game will perform the best on average. However, in the long run we may see the current championship accuracy of the consensus model start to normalize, and gradient boosted trees perform the best overall.
...
Amin, N. (2026). March Madness Data [Dataset, Public Picks.csv]. Kaggle. 
	https://www.kaggle.com/datasets/nishaanamin/march-madness-data/code. Accessed 5/1/2026.
	
Kaggle. (2026). March Machine Learning Mania 2026 competition data [Data set]. Kaggle. 
	https://www.kaggle.com/competitions/march-machine-learning-mania-2026/data. Accessed 5/1/2026
