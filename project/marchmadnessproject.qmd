...
where $\hat{p}_i$ is the predicted probability that team 1 wins game $i$, $B$ is the total number of decision trees in the forest, $T_b(X_i)$ is the predicted probability from tree $b$ using predictor vector $X_i$, and $X_i$ contains the differences in team-level statistics between the two teams.

Each tree is trained on a bootstrap sample of the data, while a random subset of predictor variables is considered at each split to reduce correlation between trees and improve predictive performance.
...
where $y_i$ is the observed outcome for game $i$, $X_i$ is the vector of predictor variables, $\beta$ is the vector of coefficients, $n$ is the number of observations, and $p$ is the number of predictor variables.

$\lambda$ controls the overall strength of regularization, while $\alpha$ determines the balance between the lasso penalty ($L_1$) and ridge penalty ($L_2$). When $\alpha = 1$, the model becomes lasso regression, and when $\alpha = 0$, the model becomes ridge regression.

For this study, Elastic Net is used to estimate matchup win probabilities while reducing multicollinearity among predictor variables and preventing over-fitting.
...
Each year this project could be updated with the new tournament data to further examine how model performance averages out over time.
