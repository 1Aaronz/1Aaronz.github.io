...
To answer this question, I will be evaluating each model on Brier-score, Log-loss, and Scored Simulated Brackets. The evaluation had nuanced results, with machine learning models generally outperforming a human-derived model, but with the human-derived model outperforming in a key metric, championship game accuracy.

# Background and Related Literature

The structure of March Madness is a 64 team, single elimination tournament.

Many people fill out brackets each year, an estimated 50 million (Lopez, M. J., & Matthews, G. J), attempting to predict which teams will move on to each round. Billions of dollars each year are gambled on the tournament. Using a model that outperforms all others would produce a significant advantage in NCAA bracket competitions.
...
$B_b$ = one complete simulated tournament bracket with random draws chosen for all 63 games
